...
# function to apply reference transformation
ref_transform <- function(Z){
  p <- ncol(Z)
  n <- nrow(Z)
  ZZ = matrix(NA,n,p-1)
  for(j in 1:(p-1)){
...
PE <- function(beta, X, beta_star) {
  ...
  diff <- as.matrix(beta - beta_star, ncol = length(beta))
  tryCatch({
    error <- t(diff) %*% t(X) %*% X %*% diff / n
    as.numeric(error)
  }, error = function(e) {
    print(paste("Dimensions of beta:", dim(beta)))
...
eval_results <- function(beta, X, beta_star, beta_test = NULL) {
  if (!is.null(beta_test)) {
    Fa <- Falsility(beta_test, beta_star)
  } else {
