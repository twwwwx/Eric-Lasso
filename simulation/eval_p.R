...
# model settings
model_list <- list()
model_list[["coco_p"]] <- c(FALSE, TRUE)
# --------------------------
...
for (p in c(100)) {
    for (i in seq_along(model_list)) {
        ...
        for (i in 1:N_sim) {
            ...
            # log -> log ratio
            # Z <- apply(data$Z, 1, function(x) x[1:(p - 1)] - x[p])
            Z <- data$Z[, 1:(p - 1)] - data$Z[, p]
            Sig_B <- data$Sig_B[1:(p - 1), 1:(p - 1)] + data$Sig_B[p, p]
            # Z <- data$Z[, 2:p] - data$Z[, 1]
            # Sig_B <- data$Sig_B[2:p, 2:p] + data$Sig_B[1, 1]

            # record time
            fit_additive <- eic(Z = Z, y = data$y, n = n, p = p - 1, scale.Z = FALSE, scale.y = FALSE, step = 100, K = 5, mu = 10, earlyStopping_max = 10, Sig_B = Sig_B, etol = 1e-4, noise = "additive", penalty = "lasso", proj = proj, constrain = constrain)

            # naive evaluation
            results_df$lambda[i] <- fit_additive$lambda.opt
            beta <- fit_additive$beta.opt
            beta <- c(beta, -sum(beta))
            # beta <- c(-sum(beta), beta)

            centered_X <- data$X - rep(c(fit_additive$mean.Z, 0), each = n)
            # centered_X <- data$X - rep(c(0,fit_additive$mean.Z), each = n)
            measures <- eval_results(beta, centered_X, beta_star)
            ...
            results_df$sum_beta[i] <- sum(beta)
        }
