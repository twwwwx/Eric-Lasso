...
cv_covariance_matrices <- function(K,
  ...
  if (noise == "additive") {
    ...
    if (mode == "ADMM") {
      sigma_global <- ADMM_proj_cpp(cov_modified, mu = mu, etol = etol)$mat
    }
    # if (mode == "HM") {
    ...
    for (i in 1:K) {
      ...
      if (mode == "ADMM") {
        mat_cov_train <- ADMM_proj_cpp(cov_modified_train, mu = mu, etol = etol)$mat
      }
      # if (mode == "HM") {
      ...
      mat_cov_test <- ADMM_proj_cpp(cov_modified_test, mu = mu, etol = etol)$mat
