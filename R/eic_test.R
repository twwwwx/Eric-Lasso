...
library(Rcpp)
library(parallel)
sourceCpp("R/lasso_covariance_constrained.cpp")
...
cross_validation_function <- function(k,
  ...
  if (constrain) {
    coef_lambda <- lasso_covariance_con_cpp(n = n, p = p, lambda = lambda_step, XX = sigma_train, Xy = rho_train, beta_start = beta_start, penalty = penalty)$coefficients
...
eic <- function(Z,
  ...
  if (proj) {
    ...
    print(paste("Time of ADMM:", Sys.time()-time))
  } else {
  ...
  for (i in 1:step) {
    ...
    # # Replace the parallel computing code with multi-threading using mclapply
    # out <- mclapply(1:K, function(k) {
    #     cross_validation_function(k,
    #       n = n,
    #       p = p,
    ...
    if (constrain) {
      ...
      coef_tot <- lasso_covariance_con_cpp(n = n, p = p, lambda = lambda_step, XX = ZZ, Xy = Zy, beta_start = beta_start, penalty = penalty)$coefficients
    } else {
