...
for (m in 1:2) {
    ...
    results_df <- data.frame(lam=numeric(N_sim), MSE = numeric(N_sim), MAE=numeric(N_sim), selected=numeric(N_sim), sum_beta = numeric(N_sim))
    support <- list()
    ...
    for (model_name in names(model_list)) {
        ...
        for (i in 1:N_sim) {
            ...
            ind <- sample(1:nrow(data$X), K - nrow(data$X) %% K)
            X.p <- rbind(data$X, data$X[ind, ])
            Z.p <- rbind(data$Z, data$Z[ind, ])
            y.p <- rbind(data$y, as.matrix(data$y[ind, ]))
            n.p <- nrow(X.p)
            center <- function(x) {
                return(x - mean(x))
            }
            B.p <- apply(Z.p,2, center) - apply(X.p,2, center)

            Sig_B <- t(B.p) %*% B.p / nrow(B.p)
            fit <- eic(Z = Z.p, y = y.p, n = n.p, p = p, scale.Z = FALSE, scale.y = FALSE, step = 100, K = K, mu = 10, earlyStopping_max = 30, Sig_B = Sig_B, etol = 1e-4, noise = "additive", penalty = "lasso", proj = model_list[[model_name]][2], constrain = model_list[[model_name]][1])
            ...
            MSE <- mean((y_pred - y.p)^2)
            MAE <- mean(abs(y_pred - y.p))
